...
function setupServiceMapPage() {
	...
	fetch('/autolock/api/lojas')
	    .then(function(res) {
	        console.log('STATUS API LOJAS:', res.status);
	        return res.json();
	    })
	    .then(function(lojas) {
	        ...
	        lojas.forEach(function(loja) {
	            var lat = parseFloat(loja.latitude);
	            var lng = parseFloat(loja.longitude);

	            if (isNaN(lat) || isNaN(lng)) {
	                console.warn('Loja sem coordenada válida:', loja);
	                return;
	            }

	            servicePlaces.push({
	                name: loja.nome || 'Loja sem nome',
	                lat: lat,
	                lng: lng,
	                type: loja.tipo || 'Oficina',
	        ...
	        if (currentPlaces.length > 0) {
	            map.fitBounds(
	                L.latLngBounds(currentPlaces.map(function(place) {
	    ...
	    .catch(function(erro) {
	        console.error('Erro ao carregar lojas da API:', erro);
	        applyServiceFilters();
	    });
...
window.buscarCEP = function () {
    ...
    fetch('https://viacep.com.br/ws/' + cep + '/json/')
        ...
        .then(function (dados) {
			...
			if (numeroInput) {
			    numeroInput.addEventListener('blur', window.buscarCEP);
			}
